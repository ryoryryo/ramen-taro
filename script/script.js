// ページがキャッシュから読み込まれてもフェードインを再適用する
...
document.addEventListener('DOMContentLoaded', function () {
    // ページ遷移アクション
    // フェードインを確実に適用するために、短い遅延を入れる
    setTimeout(() => {
        document.body.classList.add("page-fade-in"); // ページ読み込み時にフェードイン
    }, 10);

    // リンクのクリックイベントでフェードアウトを実行
    document.querySelectorAll("a[href]:not([href*='#']):not([href='javascript:void(0);'])").forEach(link => {
        link.addEventListener("click", (e) => {
            e.preventDefault(); // 通常のリンク動作を防止
            document.body.classList.remove("page-fade-in"); // フェードインを解除
            document.body.classList.add("page-fade-out"); // フェードアウト開始
    ...
    if (document.body.id === "home") {
        ...
        function handleScroll() {
            ...
            if (elmDistance <= 0) {
                ...
                subVisualWrapper.classList.add("fire");
            } else {
        ...
        // ----------------------------------------------------------
        // エンブレム出現および輝き遅延
        const homeIntoroductionSentence = document.querySelector(".home-introduction__sentence");
        const flashEmblem = document.querySelector(".flash-emblem");
        const flashEmblemLight = document.querySelector(".flash-emblem-light");

        if (homeIntoroductionSentence && flashEmblem && flashEmblemLight) {
            ...
            const observer = new IntersectionObserver(
                (entries) => {
                    entries.forEach((entry) => {
                        if (entry.isIntersecting) {
                            ...
                            flashEmblem.classList.add("is-active");

                            setTimeout(() => {
                                flashEmblemLight.classList.add("is-active");
                            }, 5000);
                        }
                    });
                },
                {
                    root: null, // ビューポートを基準
                    rootMargin: "-30% 0px", // 70% の位置を指定
                    threshold: 0, // 要素が少しでも見えたら発火
                }
            );

            // flash-emblem を監視対象に追加
            observer.observe(homeIntoroductionSentence);
        }
