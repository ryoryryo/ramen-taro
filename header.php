...
<head>
    ...
    <noscript>
        <!-- JavaScriptが無効な場合は通常のリンクを使用およびローディング画面スキップ -->
        <style>
            body {
                opacity: 1 !important;
            }
        ...
        <div class="header__wrapper">
            ...
            <nav class="header__navi">
                ...
                <?php if (!is_singular('post')) { ?>
                    <div class="language-select-box" aria-label="language menu" aria-haspopup="listbox": role="combobox">
                        Language
                        <ul>
                            <?php
                            foreach ($result['urls_and_categories'] as $item) {
                            ?>
                                <li class="<?php if ($current_lang === $item['category_slug']) {
                                                echo 'lang-selected';
                                            } ?>" <?php if ($current_lang === $item['category_slug']) {
